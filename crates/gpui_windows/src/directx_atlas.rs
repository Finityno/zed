...
impl AtlasBackend for DirectXAtlasTextures {
    fn insert(
        ...
        let uploaded = texture.upload(&self.device_context, tile.bounds, bytes);
        if !uploaded {
...
impl DirectXAtlasTexture {
    ...
    /// still blank on the GPU and must not be cached — see `AtlasBackend::insert`.
    #[must_use]
    fn upload(
        &self,
        device_context: &ID3D11DeviceContext,
...
impl DirectXAtlas {
    ...
    pub(crate) fn read_back(&self, id: AtlasTextureId) -> Option<TextureReadback> {
        ...
        let lock = self.0.lock();
        let textures = &lock.backend;
        let texture = textures.texture(id)?;
        ...
        unsafe {
            textures.device_context.CopyResource(&staging, &texture.texture);
            ...
            textures.device_context.Unmap(&staging, 0);
        }
...
mod tests {
    ...
    fn test_rejected_upload_is_not_cached_and_frees_its_tile() {
        ...
        let result = atlas.get_or_insert_with(key.clone(), &mut short_upload);
        assert!(
            result.is_err(),
