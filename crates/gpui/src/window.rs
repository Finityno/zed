...
use crate::{
    ...
    WindowBounds, WindowControls, WindowDecorations, WindowGlassStyle, WindowOptions, WindowParams,
...
pub(crate) fn text_shimmer_disabled() -> bool {
...
impl Window {
    ...
    ///
    /// Benchmarks built on `TestAppContext` drive drawing synchronously rather
    /// than through a platform frame-request loop, so they call this after
    /// each measured update to submit the frame like production presentation
    /// would.
...
mod tests {
    ...
    use crate::{
        ...
        WindowOptions, canvas, div, point, px, rgb, size, util::MIN_RETAINED_CAPACITY,
    };
    use crate::arena::SHRINK_AFTER_DURATION;
