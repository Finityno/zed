...
mod shader_tests {
    ...
    /// unlike the Metal path, whose header `gpui_macos/build.rs` generates with
    /// cbindgen. When they drift, wgpu either rejects the bind group ("bound
    /// with size N where the shader expects M", which invalidates the whole
    /// command buffer and blanks the window) or, for batches large enough to
    /// clear the minimum, silently strides through the instance buffer at the
    /// wrong pitch. WGSL's alignment rules are the trap: `vec3<u32>` aligns to
    /// 16 where Rust's `[u32; 3]` aligns to 4, and one such field re-pads every
    ...
    fn sprite_layouts_match_rust() {
        ...
        let module = naga::front::wgsl::parse_str(&source).expect("shaders parse");
        ...
        let expected: &[(&str, usize)] = &[
            ...
            ("Quad", size_of::<gpui::Quad>()),
            ("Shadow", size_of::<gpui::Shadow>()),
            ("Underline", size_of::<gpui::Underline>()),
            ("MonochromeSprite", size_of::<gpui::MonochromeSprite>()),
            ("PolychromeSprite", size_of::<gpui::PolychromeSprite>()),
            ("SubpixelSprite", size_of::<gpui::SubpixelSprite>()),
        ];
